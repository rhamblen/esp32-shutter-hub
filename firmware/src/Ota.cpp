...
// Firmware flashes reboot the device (the new app only runs after a restart);
// filesystem flashes do not.

namespace {
bool   g_ok      = false;   // result of the in-flight / last upload
String g_err     = "";
String g_target  = "firmware";
bool   g_reboot  = false;   // set after a successful firmware flash, serviced in loop()
}
...
void begin(AsyncWebServer &server) {
  server.on("/api/ota", HTTP_POST,
    ...
    [](AsyncWebServerRequest *req) {
      ...
      if (g_ok && g_target == "firmware") g_reboot = true;  // new app needs a restart
    },
...
void loop() {
  if (g_reboot) {
    g_reboot = false;
    LOGW("ota", "rebooting into new firmware");
    delay(600);              // let the HTTP response flush first
    ESP.restart();
  }
}
